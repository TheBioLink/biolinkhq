...
export async function PATCH(req) {
  ...
  if (action === "report") {
    const reporterPage = await Page.findOne({ owner: meEmail }).lean();
    ...
    const messageLog = fullMessages.map((m) => ({
      fromUsername: m.fromEmail === meEmail ? reporterPage?.uri || "reporter" : targetPage.uri,
    ...
    await MessageReport.create({
      ...
      reporterUsername: reporterPage?.uri || "",
      reportedEmail: otherEmail,
      reportedUsername: targetPage.uri,
      reason,
      messageLog,
      recentMessages: messageLog.slice(-10),
    });
    return NextResponse.json({ ok: true });
  }
